(function($) {
    $.fn.richText = function(options) {
        ...
        var $editor, $toolbarList = $("<ul />"),
            ...
            $btnCode = $("<a />", { class: "richText-btn", "data-command": "toggleCode", title: settings.translations.code, html: '<span class="fa fa-code"></span>' });
        ...
        for (var i = 0; i < fonts.length; i++) { $fonts.append($("<li />", { html: '<a style="font-family:' + fonts[i] + ';" data-command="fontName" data-option="' + fonts[i] + '">' + fonts[i] + "</a>" })) }
        ...
        for (var i = 0; i < fontSizes.length; i++) { $fontSizes.append($("<li />", { html: '<a style="font-size:' + fontSizes[i] + 'px;" data-command="fontSize" data-option="' + fontSizes[i] + '">' + settings.translations.text + " " + fontSizes[i] + "px</a>" })) }
        ...
        $linksForm.append($formItem.clone().append($formLabel.clone().text(settings.translations.openIn).attr("for", "openIn")).append($formInputSelect.clone().attr("id", "openIn").append($("<option />", { value: "_self", text: settings.translations.sameTab })).append($("<option />", { value: "_blank", text: settings.translations.newTab }))));
        ...
        if (settings.imageHTML && ($(settings.imageHTML).find("#imageURL").length > 0 || $(settings.imageHTML).attr("id") === "imageURL")) { $imageForm.html(settings.imageHTML) } else {
        ...
        if (settings.fileHTML && ($(settings.fileHTML).find("#fileURL").length > 0 || $(settings.fileHTML).attr("id") === "fileURL")) { $fileForm.html(settings.fileHTML) } else {
        ...
        function init() {
            ...
            if (settings.useParagraph !== false) { document.execCommand("DefaultParagraphSeparator", false, "p") }
            if ($inputElement.prop("tagName") === "TEXTAREA") {} else if ($inputElement.val()) {
                value = $inputElement.val();
                attributes = $inputElement.prop("attributes");
                $.each(attributes, function() { if (this.name) { attributes_html += " " + this.name + '="' + this.value + '"' } });
            ...
            if (tabindex >= 0 && settings.useTabForNext === true) { $editorView.attr("tabindex", tabindex) }
            $toolbar.append($toolbarList);
            settings.$editor = $editor;
            if (settings.bold === true) { $toolbarList.append($toolbarElement.clone().append($btnBold)) }
            if (settings.italic === true) { $toolbarList.append($toolbarElement.clone().append($btnItalic)) }
            if (settings.underline === true) { $toolbarList.append($toolbarElement.clone().append($btnUnderline)) }
            if (settings.leftAlign === true) { $toolbarList.append($toolbarElement.clone().append($btnLeftAlign)) }
            if (settings.centerAlign === true) { $toolbarList.append($toolbarElement.clone().append($btnCenterAlign)) }
            if (settings.rightAlign === true) { $toolbarList.append($toolbarElement.clone().append($btnRightAlign)) }
            if (settings.justify === true) { $toolbarList.append($toolbarElement.clone().append($btnJustify)) }
            if (settings.ol === true) { $toolbarList.append($toolbarElement.clone().append($btnOL)) }
            if (settings.ul === true) { $toolbarList.append($toolbarElement.clone().append($btnUL)) }
            if (settings.fonts === true && settings.fontList.length > 0) { $toolbarList.append($toolbarElement.clone().append($btnFont)) }
            if (settings.fontSize === true) { $toolbarList.append($toolbarElement.clone().append($btnFontSize)) }
            if (settings.heading === true) { $toolbarList.append($toolbarElement.clone().append($btnHeading)) }
            if (settings.fontColor === true) { $toolbarList.append($toolbarElement.clone().append($btnFontColor)) }
            if (settings.imageUpload === true) { $toolbarList.append($toolbarElement.clone().append($btnImageUpload)) }
            if (settings.fileUpload === true) { $toolbarList.append($toolbarElement.clone().append($btnFileUpload)) }
            if (settings.videoEmbed === true) { $toolbarList.append($toolbarElement.clone().append($btnVideoEmbed)) }
            if (settings.urls === true) { $toolbarList.append($toolbarElement.clone().append($btnURLs)) }
            if (settings.table === true) { $toolbarList.append($toolbarElement.clone().append($btnTable)) }
            if (settings.removeStyles === true) { $toolbarList.append($toolbarElement.clone().append($btnRemoveStyles)) }
            if (settings.code === true) { $toolbarList.append($toolbarElement.clone().append($btnCode)) }
            $editorView.html($inputElement.val());
            $editor.append($toolbar);
            $editor.append($editorView);
            $editor.append($inputElement.clone().hide());
            $inputElement.replaceWith($editor);
            $editor.append($("<div />", { class: "richText-toolbar" }).append($("<a />", { class: "richText-undo is-disabled", html: '<span class="fa fa-undo"></span>', title: settings.translations.undo })).append($("<a />", { class: "richText-redo is-disabled", html: '<span class="fa fa-repeat fa-redo"></span>', title: settings.translations.redo })).append($("<a />", { class: "richText-help", html: '<span class="fa fa-question-circle"></span>' })));
            ...
            if (settings.height && settings.height > 0) { $editor.children(".richText-editor, .richText-initial").css({ "min-height": settings.height + "px", height: settings.height + "px" }) } else if (settings.heightPercentage && settings.heightPercentage > 0) {
            ...
            if (settings.callback && typeof settings.callback === "function") { settings.callback($editor) }
        }
        init();
        settings.$editor.find(".richText-help").on("click", function() {
            ...
            if ($editor) {
                ...
                $editor.append($outer);
                $outer.on("click", "#closeHelp", function() { $(this).parents(".richText-help-popup").remove() })
            }
        });
        settings.$editor.find(".richText-undo, .richText-redo").on("click", function() { var $this = $(this); if ($this.hasClass("richText-undo") && !$this.hasClass("is-disabled")) { undo(settings.$editor) } else if ($this.hasClass("richText-redo") && !$this.hasClass("is-disabled")) { redo(settings.$editor) } });
        settings.$editor.find(".richText-editor").on("input change blur keydown keyup", function(e) {
            if ((e.keyCode === 9 || e.keyCode === "9") && e.type === "keydown") {
                if (settings.useTabForNext === true) { focusNextElement(); return false }
                e.preventDefault();
        ...
        settings.$editor.find(".richText-editor").on("contextmenu", ".richText-editor", function(e) {
            ...
            if ($target.prop("tagName") === "A") {
                ...
                $list.find(".fa-image").on("click", function() {
                    var align;
                    if ($target.parent("div").length > 0 && $target.parent("div").attr("style") === "text-align:center;") { align = "center" } else { align = $target.attr("align") }
                    $(".list-rightclick.richText-list").remove();
                    $target.addClass("richText-editNode");
        ...
        settings.$editor.find(".richText-initial").on("input change blur", function() {
            if (settings.useSingleQuotes === true) { $(this).val(changeAttributeQuotes($(this).val())) }
            var editorID = $(this).siblings(".richText-editor").attr("id");
            updateEditor(editorID);
            doSave(editorID);
            updateMaxLength(editorID)
        });
        settings.$editor.find(".richText-editor").on("dblclick mouseup", function() { doSave($(this).attr("id")) });
        settings.$editor.find("#richText-Video button.btn").on("click", function(event) {
            ...
            if ($form.attr("data-editor") === editorID) {
                ...
                if (!url) {
                    ...
                    setTimeout(function() { $form.children(".form-item.is-error").slideUp(function() { $(this).remove() }) }, 5e3)
                } else {
                    var html = "";
        ...
        $(document).on("mousedown", function(e) {
            ...
            if ($target.prop("tagName") === "IMG" && $target.parents("#" + editorID)) {
                ...
                if ((resize === true || $resizeImage) && !$resizeImage.data("width")) {
                    ...
                    e.preventDefault()
                } else if (resize === true || $resizeImage) { e.preventDefault() } else { $resizeImage = null }
            }
        });
        $(document).mouseup(function() {
            if ($resizeImage) { $resizeImage.css({ cursor: "default" }) }
            $resizeImage = null
        ...
        settings.$editor.find("#richText-URL button.btn").on("click", function(event) {
            ...
            if ($form.attr("data-editor") === editorID) {
                ...
                if (!url) {
                    ...
                    var html = "";
                    if (settings.useSingleQuotes === true) { html = "<a href='" + url + "' target='" + target + "'>" + text + "</a>" } else { html = '<a href="' + url + '" target="' + target + '">' + text + "</a>" }
                    ...
                    if ($editNode.length > 0 && $editNode.prop("tagName") === "A") {
                        ...
                        if ($editNode.attr("class") === "") { $editNode.removeAttr("class") }
                    } else { pasteHTMLAtCaret(html) }
        ...
        settings.$editor.find("#richText-Image button.btn").on("click", function(event) {
            ...
            if ($form.attr("data-editor") === editorID) {
                ...
                if (!url) {
                    ...
                    var html = "";
                    if (settings.useSingleQuotes === true) { if (align === "center") { html = "<div style='text-align:center;'><img src='" + url + "'></div>" } else { html = "<img src='" + url + "' align='" + align + "'>" } } else { if (align === "center") { html = '<div style="text-align:center;"><img src="' + url + '"></div>' } else { html = '<img src="' + url + '" align="' + align + '">' } }
                    restoreSelection(editorID, true);
                    var $editNode = $(".richText-editNode");
                    if ($editNode.length > 0 && $editNode.prop("tagName") === "IMG") {
                        ...
                        } else { $editNode.attr("align", align) }
                        $editNode.removeClass("richText-editNode");
                        if ($editNode.attr("class") === "") { $editNode.removeAttr("class") }
                    } else { pasteHTMLAtCaret(html) }
        ...
        settings.$editor.find("#richText-File button.btn").on("click", function(event) {
            ...
            if ($form.attr("data-editor") === editorID) {
                ...
                if (!url) {
                    ...
                    var html = "";
                    if (settings.useSingleQuotes === true) { html = "<a href='" + url + "' target='_blank'>" + text + "</a>" } else { html = '<a href="' + url + '" target="_blank">' + text + "</a>" }
                    restoreSelection(editorID, true);
                    pasteHTMLAtCaret(html);
        ...
        settings.$editor.find("#richText-Table button.btn").on("click", function(event) {
            ...
            if ($form.attr("data-editor") === editorID) {
                ...
                if (!rows || rows <= 0) { rows = 2 }
                if (!columns || columns <= 0) { columns = 2 }
                var html = "";
                if (settings.useSingleQuotes === true) { html = "<table class='table-1'><tbody>" } else { html = '<table class="table-1"><tbody>' }
                for (var i = 1; i <= rows; i++) {
                    html += "<tr>";
                    for (var n = 1; n <= columns; n++) { html += "<td> </td>" }
                    html += "</tr>"
                }
                html += "</tbody></table>";
                restoreSelection(editorID, true);
        ...
        $(document).on("click", function(event) {
            ...
            if ($clickedElement.parents(".richText-toolbar").length === 0) {} else if ($clickedElement.hasClass("richText-dropdown-outer")) { $clickedElement.parent("a").parent("li").removeClass("is-selected") } else if ($clickedElement.find(".richText").length > 0) { $(".richText-toolbar li").removeClass("is-selected") } else if ($clickedElement.parent().hasClass("richText-dropdown-close")) { $(".richText-toolbar li").removeClass("is-selected") } else if ($clickedElement.hasClass("richText-btn") && $(event.target).children(".richText-dropdown-outer").length > 0) {
                ...
                if ($clickedElement.children(".fa,svg").hasClass("fa-link")) {
                    ...
                    if (selectedText) { $clickedElement.find("input#urlText").val(selectedText) }
        ...
        settings.$editor.find(".richText-toolbar a[data-command]").on("click", function(event) {
            ...
            if ($editor.length > 0 && id === editorID && (!$button.parent("li").attr("data-disable") || $button.parent("li").attr("data-disable") === "false")) {
                ...
                if (command === "toggleCode") { toggleCode($editor.attr("id")) } else {
                    var option = null;
                    if ($(this).data("option")) { option = $(this).data("option").toString(); if (option.match(/^h[1-6]$/)) { command = "heading" } }
                    ...
                    if (command === "removeFormat") {
                        $editor.find("*").each(function() {
                            ...
                            $.each(attributes, function(i, item) { if (keepAttributes.indexOf(item) < 0 && item.substr(0, 5) !== "data-") { element.removeAttr(item) } });
                            if (element.prop("tagName") === "A") { element.replaceWith(function() { return $("<span />", { html: $(this).html() }) }) }
                        });
        ...
        function focusNextElement() {
            ...
            if (document.activeElement) {
                var focussable = Array.prototype.filter.call(document.querySelectorAll(focussableElements), function(element) { return element.offsetWidth > 0 || element.offsetHeight > 0 || element === document.activeElement });
        ...
        function formatText(command, option, editorID) {
            ...
            doRestore(editorID);
            if (command === "heading" && getSelectedText()) { pasteHTMLAtCaret("<" + option + ">" + getSelectedText() + "</" + option + ">") } else if (command === "fontSize" && parseInt(option) > 0) {
                ...
                pasteHTMLAtCaret(html)
            } else { document.execCommand(command, false, option) }
        }

        function updateTextarea() {
            var $editor = $("#" + editorID);
            var content = $editor.html();
            if (settings.useSingleQuotes === true) { content = changeAttributeQuotes(content) }
        ...
        function restoreSelection(editorID, media, url) {
            ...
            if (!savedSel) { savedSel = { start: 0, end: 0, type: "caret", editorID: editorID, anchor: $("#" + editorID).children("div")[0] } }
            if (savedSel.editorID !== editorID) { return false } else if (media === true) { containerEl = savedSel.anchor ? savedSel.anchor : containerEl } else if (url === true) { if (savedSel.start === 0 && savedSel.end === 0) { containerEl = savedSel.anchor ? savedSel.anchor : containerEl } }
            if (window.getSelection && document.createRange) {
                var charIndex = 0,
                ...
                while (!stop && (node = nodeStack.pop())) {
                    ...
                    } else { var i = node.childNodes.length; while (i--) { nodeStack.push(node.childNodes[i]) } }
                }
            ...
            if (win.getSelection) {
                ...
                if (sel.rangeCount > 0) {
                    ...
                    if (textNode != null) {
                        ...
                        if (textNode.textContent != null) { sel.selectAllChildren(textNode) }
                        e.preventDefault();
                        return true
                        ...
                        var $tr = $("<tr />");
                        var $td = $("<td />");
                        for (var i = 1; i <= cellsPerLine; i++) { $tr.append($td.clone()) }
                        $table.append($tr);
        ...
        function getSelectedText() { var range; if (window.getSelection) { range = window.getSelection(); return range.toString() ? range.toString() : range.focusNode.nodeValue } else if (document.selection.createRange) { range = document.selection.createRange(); return range.text } return false }
        ...
        function setHistoryButtons(id) { if (historyPosition[id] <= 0) { $editor.find(".richText-undo").addClass("is-disabled") } else { $editor.find(".richText-undo").removeClass("is-disabled") } if (historyPosition[id] >= history[id].length - 1 || history[id].length === 0) { $editor.find(".richText-redo").addClass("is-disabled") } else { $editor.find(".richText-redo").removeClass("is-disabled") } }

        function undo($editor) {
            var id = $editor.children(".richText-editor").attr("id");
            historyPosition[id]--;
            if (!historyPosition[id] && historyPosition[id] !== 0) { return false }
        ...
        function changeAttributeQuotes(string) {
            ...
            if (settings.useSingleQuotes === true) {
                regex = /\s+(\w+\s*=\s*(["][^"]*["])|(['][^']*[']))+/g;
                rstring = string.replace(regex, function($0, $1, $2) { if (!$2) { return $0 } return $0.replace($2, $2.replace(/\"/g, "'")) })
            } else {
                regex = /\s+(\w+\s*=\s*(['][^']*['])|(["][^"]*["]))+/g;
                rstring = string.replace(regex, function($0, $1, $2) { if (!$2) { return $0 } return $0.replace($2, $2.replace(/'/g, '"')) })
            }
        ...
        function loadColors(command) {
            ...
            if (settings.colors && settings.colors.length > 0) { colors = settings.colors }
        ...
        function toggleCode(editorID) {
            ...
            if ($editor.find(".richText-editor").is(":visible")) {
                ...
                $(".richText-toolbar").find(".richText-btn").each(function() { if ($(this).children(".fa-code").length === 0) { $(this).parent("li").attr("data-disable", "true") } });
        ...
        function convertCaretPosition(editorID, selection, reverse) {
            ...
            if (!selection || !code) { return { start: 0, end: 0 } }
            if (reverse === true) {
            ...
            for (var i = 0; i < code.length; i++) {
                if (code[i] === "<") {
                    ...
                    states.tag = false;
                    states.tagsCount++
                } else if (states.isTag === true && code[i] !== ">") { states.tagsCount++ } else if (states.isTag === true && code[i] === ">") {
                    states.isTag = false;
                    states.tag = true;
                    states.tagsCount++
                } else if (states.tag === true) { states.tag = false }
                if (!reverse) {
        ...
        function getVideoCode(url, size) {
            ...
            if (!size) {
                size = "640x360";
                size = size.split("x")
            } else if (size !== "responsive") { size = size.split("x") } else {
                responsive = true;
                size = "640x360";
                size = size.split("x")
            }
            var html = "<br><br>";
            if (responsive === true) { html += '<div class="videoEmbed" style="position:relative;height:0;padding-bottom:56.25%">' }
        ...
        function fixFirstLine() {
            if ($editor && !$editor.find(".richText-editor").html()) { if (settings.useParagraph !== false) { $editor.find(".richText-editor").html("<p><br></p>") } else { $editor.find(".richText-editor").html("<div><br></div>") } } else { if (settings.useParagraph !== false) { $editor.find(".richText-editor").find("div:not(.videoEmbed)").replaceWith(function() { return $("<p />", { html: $(this).html() }) }) } else { $editor.find(".richText-editor").find("p").replaceWith(function() { return $("<div />", { html: $(this).html() }) }) } }
            updateTextarea()
        }
        return $(this)
    };
    $.fn.unRichText = function(options) {
        ...
        function init() {
            if ($el.hasClass("richText")) { $main = $el } else if ($el.hasClass("richText-initial") || $el.hasClass("richText-editor")) { $main = $el.parents(".richText") }
            if (!$main) { return false }
            $editor = $main.find(".richText-editor");
            $textarea = $main.find(".richText-initial");
            if (parseInt(settings.delay) > 0) { setTimeout(remove, parseInt(settings.delay)) } else { remove() }
        }
        init();
